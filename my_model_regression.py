...
ds=MyMolProtDataset("../data/davis_deal.txt")
#ds=MyMolProtDataset("../data/davis_deal.txt")
model=MyModel()
...
def test(ds):
    ...
    for batch in test_loader:
        ...
        y_pred=lr.predict(s5)

        R_score,p=pearsonr(t4,s4)
        r2=r2_score(t4,s4)
        ...
        plt.figure()
        # plt.scatter(s4,t4,c='r')
        plt.scatter(S,T,c='r')
        plt.plot(s5,y_pred,c='b',label='R score %.3f'%(R_score))
        plt.legend()
        plt.xlabel('label combination')
        plt.ylabel('predicted combination')
