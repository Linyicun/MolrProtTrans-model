...
class Predictor(nn.Module):
    ...
    def __call__(self, data, train=True):
        ...
        if train:
            ...
            x=predicted_interaction[:,0]

            #print(correct_interaction)
            correct_interaction=correct_interaction.unsqueeze(1)
            # print(predicted_interaction.dtype)
            # print(correct_interaction.dtype)
            ...
            return correct_labels, predicted_interaction
    ...
    def test(self, dataset):
        ...
        with torch.no_grad():
            for data in dataset:
                ...
                correct_labels, predicted_intereactions = self.model(data, train=False)
                
                S.append([predicted_intereactions.cpu()[0][0].item()])
            ...
            plt.figure()
            plt.scatter(S,T,c='r')
            plt.plot(S,y_pred,c='b',label='R score %.3f'%(pearsonr(T,y_pred)[0]))
            plt.legend()
            plt.savefig("regression_transformer_davis.jpg")
        return R_score

        

    def save_Rs(self, Rs, filename):
