...
ds=MyMolProtDataset("../data/GPCR_deal.txt")
model=MyModel()
...
for epoch in range(3):
    for batch in loader:
        ...
        f_train_my_model=open("train_my_model.txt","a")
        AUC,PRC=test(model,loader)
        f_train_my_model.write(str(AUC)+str(PRC)+"\n")
        f_train_my_model.close()
    ...
    for batch in test_loader:
        ...
        f_roc_data=open('my_model_GPCR_ROC_data.txt',"w")
        f_roc_data.write(str(fr)+"\n")
        f_roc_data.write(str(tr)+"\n")
        f_roc_data.close()

        f_result=open('roc_result.txt','w')
        f_result.write(str(fr)+'\n'+str(tr)+'\n'+str(r_a)+'\n')
        f_result.close()
